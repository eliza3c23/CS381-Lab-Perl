...
sub extract_title{
	...
	foreach my $track (@tracks) { 
		...
		##matches any char<SEP> for 0 - unlimted times + group 1 aka song title
		##
		if ($track =~ /.*<SEP>(.*)/){
		##		
		############################# End Task 1
...
sub clean{
	...
	foreach my $title (@tracks) { 
		...
		# ^ start of string/line eg ^abc.* -> abc(line start)
		# $ end of string/line eg *?the end$ -> this is the end
		# $title =~ s/^[\']+$//;
		$title =~ s/^'+//g; 
		$title =~ s/'+$//g;
		...
		#POSIX char classes[[:alnum:]],[[:space:]](upper and lower letters, digit, space)
		#word,space,apostophe OR non-ASCII
		# next if($title =~ /[^\[:word:]]/);
		next if($title =~ m/[^\[[:alnum:]\]|[[:space:]\|\']|\[|\]|\_|\.|\|/g);
		########################## End Task 5
...
sub stopwords{
	...
	# s/\b(stopword)\b\s//g
	foreach my $title (@tracks){
		$title =~ s/\b(a|an|and|by|for|from|in|of|on|or|out|the|to|with)\b\s//g;
		push @filteredtitles, $title;
	}
	########################## End Task 8
...
sub build_bigrams{
	...
	foreach $title (@tracks){
		# split title into words. \s+ whitespace
		my @titleWord = split(/\s+/,$title);
		
		# create new array titleWord
		for(my $i = 0; $i < $#titleWord; $i++){
			# check if key already exists
			if(!exists($counts{$titleWord[$i + 1]})){
				$counts{$titleWord[$i]}{$titleWord[$i + 1]} = 1;
			}
			else{
				$counts{$titleWord[$i]}{$titleWord[$i + 1]} ++;
			}	
		}
	}   
...
sub mcw{
	...
	# sort words in counts
	##########################
	# Temp helper value
	my $t = 0;
	#%{$counts{$words}}
	foreach my $wordt (sort keys %counts)
	{
		if ($counts{$word}{$wordt} > $t)
		{
			# check word_history, make sure it is not used
			if($word_history{$wordt} != 1)
			{
				$t = $counts{$word}{$wordt};
				# update best_word
				$best_word = $wordt;
...
sub sequence{
	...
	##########################
	##########################
	# Seed word (arg) for sequence
	my $seq = $_[0];
	my @newArr = [];
	my $i = 1;
	$word_history{$newArr[$0]} = 1;

	# while $i < SEQUENCE_LENGTH
	@newArr = $_[0];
	for($i = 1; $i < $SEQUENCE_LENGTH ; $i++){
		# add to sequence
		push(@newArr,mcw(@newArr[$i - 1]));
		# track word history
		$word_history{$newArr[$i]} = 1;
		$seq = "$seq @newArr[$i]";
